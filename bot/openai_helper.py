...
class OpenAIHelper:
    ...
    async def generate_image(self, prompt: str):
        ...
        if not hf_token:
            raise ValueError("未在环境变量中找到 HF_TOKEN")
        ...
        try:
            # 强制指定 provider="fal-ai" 确保路由到支持图像生成的节点
            ...
            # 将返回的 PIL.Image 转换为 PNG 字节流发给 Telegram
            image_bytes = io.BytesIO()
            image.save(image_bytes, format="PNG")
            image_bytes.seek(0)
            image_bytes.name = "generated.png"

            return image_bytes, "1024x1024"

        except Exception as e:
            raise Exception(f"⚠️ Hugging Face 画图失败: {str(e)}") from e
    ...
    def __max_model_tokens(self):
        ...
        # 删除原本的 raise 报错，替换为默认放行长度（注意和上方的 if 保持相同的代码缩进）：
        return 128000
